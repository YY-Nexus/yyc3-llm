...
export class CodeCompiler {
  ...
  private validateDataFlow(warnings: CompileWarning[], _errors: CompileError[]): void {
    // 检查必需输入是否已连接
    for (const node of this.project.nodes) {
  ...
  private validateComponentCompatibility(warnings: CompileWarning[], _errors: CompileError[]): void {
...
.visual-app > * + * {
  ...
  private extractStateFields(): Array<{name: string, type: string, defaultValue: unknown}> {
    const fields: Array<{name: string, type: string, defaultValue: unknown}> = []
