...
export class VisualProgrammingEngine {
  ...
  private inferEdgeType(sourceNodeId: string, sourcePortId: string, targetNodeId: string, _targetPortId: string): VisualEdge['type'] {
    const sourceNode = this.project.nodes.find(n => n.id === sourceNodeId)
    const targetNode = this.project.nodes.find(n => n.id === targetNodeId)
...
${jsx}
  ...
  private generateVanillaCode(): string {
    // 原生JavaScript代码生成实现
    return `// Generated vanilla JavaScript code
console.log('Vanilla code placeholder');`
  }
  ...
  // 属性读取类型守卫
  private getStringProp(node: VisualNode, key: string, fallback = ''): string {
    const val = node.properties[key]
    return typeof val === 'string' ? val : fallback
  }

  private getNumberProp(node: VisualNode, key: string, fallback = 0): number {
    const val = node.properties[key]
  ...
  private emit(eventType: string, data: unknown): void {
    ...
    this.eventEmitter.dispatchEvent(new CustomEvent(eventType, { detail: safeData }))
  }
  ...
  private initializeBuiltinNodes(): void {
    ...
    this.registerNode('button', {
      ...
      generateJSX: (node, _project) => {
        const clickHandler = this.findConnectedHandler(node.id, 'click', _project)
        const variant = this.getStringProp(node, 'variant', 'primary')
        const size = this.getStringProp(node, 'size', 'medium')
        const text = this.getStringProp(node, 'text', '点击我')
        return `      <Button 
    ...
    this.registerNode('input', {
      ...
      generateJSX: (node, _project) => {
        const placeholder = this.getStringProp(node, 'placeholder', '请输入...')
        const type = this.getStringProp(node, 'type', 'text')
        return `      <Input 
    ...
    this.registerNode('emotion-detector', {
      ...
      generateJSX: (node, _project) => {
        const mode = this.getStringProp(node, 'mode', 'text')
        const sensitivity = this.getNumberProp(node, 'sensitivity', 0.7)
    ...
    this.registerNode('ai-assistant', {
      ...
      generateJSX: (node, _project) => {
        const model = this.getStringProp(node, 'model', 'gpt-4')
        const maxTokens = this.getNumberProp(node, 'maxTokens', 1000)
        const temperature = this.getNumberProp(node, 'temperature', 0.7)
        return `      <AIAssistant 
        model="${model}"
        maxTokens={${maxTokens}}
