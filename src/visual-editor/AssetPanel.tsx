...
export const AssetPanel: React.FC<AssetPanelProps> = ({ assets, onAddComponent }) => {
  ...
  return (
    <aside className="w-64 min-w-[180px] bg-gradient-to-b from-blue-50 to-blue-100 border-r p-4 flex flex-col gap-4 rounded-r-2xl shadow-lg">
      ...
      <div className="flex flex-col gap-3">
        {assets.map((asset) => (
          <div key={asset.id} className="transition-all duration-150">
            <DraggableAsset asset={asset} onAddComponent={onAddComponent} />
          </div>
        ))}
      </div>
      {showAI && (
          ...
          <div className="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full relative">
            ...
            <div className="mb-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {currentRecommend.map((item: { name: string; type: string }, idx: number) => (
