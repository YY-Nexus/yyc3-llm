...
import { AICodeAssistant, RealTimeCollaboration, ProgressVisualization } from '../education/AdvancedIntegration'
import { EducationFeatureModal, EducationQuickEntry } from '../education/EducationModal'

import { AssetPanel } from './AssetPanel'
import { CanvasArea } from './CanvasArea'
import { PropertyPanel } from './PropertyPanel'
import { useEditor } from './hooks/useEditor'
import { addComponent, updateComponent, deleteComponent, selectComponent } from './store/visualEditorSlice'
...
// 定义配置对象
const yiJiaoConfig: Record<string, { components: { name: string; type: string }[] }> = {
  '小学': { components: [{ name: "按钮", type: "button" }, { name: "输入框", type: "input" }, { name: "文本", type: "text" }] },
  '初中': { components: [{ name: "按钮", type: "button" }, { name: "输入框", type: "input" }, { name: "文本", type: "text" }, { name: "图片", type: "image" }] }
};

const gaoJiaoConfig: Record<string, { components: { name: string; type: string }[] }> = {
  '高中': { components: [{ name: "按钮", type: "button" }, { name: "输入框", type: "input" }, { name: "文本", type: "text" }, { name: "图片", type: "image" }, { name: "表单", type: "form" }] },
  '大学': { components: [{ name: "按钮", type: "button" }, { name: "输入框", type: "input" }, { name: "文本", type: "text" }, { name: "图片", type: "image" }, { name: "表单", type: "form" }, { name: "图表", type: "chart" }] }
};

// 定义缺失的组件
const EducationModes: React.FC<{ onModeSelect: (config: EducationConfig) => void }> = ({ onModeSelect }) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-xl p-6 max-w-md w-full">
        <h3 className="text-xl font-bold text-center mb-6">选择教育模式</h3>
        <div className="grid grid-cols-2 gap-4">
          <div className="border p-4 rounded-lg cursor-pointer hover:bg-blue-50" onClick={() => onModeSelect({ mode: '義教', level: '小学', userType: '学生', features: ['基础', '趣味'] })}>
            <h4 className="font-bold mb-2">义务教育 - 小学</h4>
            <p className="text-sm text-gray-600">适合小学生的基础编程</p>
          </div>
          <div className="border p-4 rounded-lg cursor-pointer hover:bg-blue-50" onClick={() => onModeSelect({ mode: '義教', level: '初中', userType: '学生', features: ['基础', '进阶'] })}>
            <h4 className="font-bold mb-2">义务教育 - 初中</h4>
            <p className="text-sm text-gray-600">适合初中生的进阶编程</p>
          </div>
          <div className="border p-4 rounded-lg cursor-pointer hover:bg-blue-50" onClick={() => onModeSelect({ mode: 'gaoJiao', level: '高中', userType: '学生', features: ['专业', '项目'] })}>
            <h4 className="font-bold mb-2">高等教育 - 高中</h4>
            <p className="text-sm text-gray-600">适合高中生的专业编程</p>
          </div>
          <div className="border p-4 rounded-lg cursor-pointer hover:bg-blue-50" onClick={() => onModeSelect({ mode: 'gaoJiao', level: '大学', userType: '学生', features: ['专业', '高级', '科研'] })}>
            <h4 className="font-bold mb-2">高等教育 - 大学</h4>
            <p className="text-sm text-gray-600">适合大学生的高级编程</p>
          </div>
        </div>
      </div>
    </div>
...
const StudentFeatures: React.FC<{ educationLevel: string; mode: string }> = ({ educationLevel, mode }) => {
  return (
    <div className="bg-blue-50 p-3 rounded-lg">
      <h4 className="font-bold text-blue-800">👨‍🎓 学生功能区</h4>
      <p className="text-sm text-gray-600">当前学习阶段：{mode} - {educationLevel}</p>
    </div>
  );
};

const TeacherFeatures: React.FC<{ educationLevel: string; mode: string }> = ({ educationLevel, mode }) => {
  return (
    <div className="bg-green-50 p-3 rounded-lg">
      <h4 className="font-bold text-green-800">👨‍🏫 教师功能区</h4>
      <p className="text-sm text-gray-600">当前教学阶段：{mode} - {educationLevel}</p>
    </div>
  );
};

const StudentTeacherFeatures: React.FC<{ isStudent: boolean; educationLevel: string; mode: string }> = ({ isStudent, educationLevel, mode }) => {
  return (
    <div className="p-4">
      {isStudent ? (
        <StudentFeatures educationLevel={educationLevel} mode={mode} />
      ) : (
        <TeacherFeatures educationLevel={educationLevel} mode={mode} />
      )}
    </div>
  );
};
...
function generateCode(canvasData: EditorComponent[], format: string) {
  switch (format) {
    case 'react':
...
export const VisualEditor: React.FC = () => {
  ...
  const { components, selectedComponentId } = useSelector((state: { visualEditor: VisualEditorState }) => state.visualEditor)
  // 将组件对象转换为数组
  const canvasData = Object.values(components || {}) as EditorComponent[]
  const selectedAsset = selectedComponentId && components[selectedComponentId] ? components[selectedComponentId] : null
  ...
  useEditor({ canvasRef: canvasRef as React.RefObject<HTMLDivElement> })
  ...
  const handleSelect = (asset: EditorComponent | null) => {
    if (asset) {
      dispatch(selectComponent(asset.id))
    } else {
  ...
  const handleUpdateComponent = (id: string, updates: Partial<EditorComponent>) => {
    dispatch(updateComponent({ componentId: id, updates }));
  };
  
  // 添加组件的处理函数
  const handleAddComponent = (component: Omit<EditorComponent, 'id'>) => {
    ...
    <div className="flex flex-col h-screen bg-slate-50">
          ...
          <div className="bg-white shadow-sm border-b p-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex gap-2">
                {educationConfig.features.map((feature: string, idx: number) => (
        ...
        <div className="flex flex-1 overflow-hidden">
          <div className="flex flex-col">
            <AssetPanel assets={getEducationAssets()} onAddComponent={handleAddComponent} />
        ...
        {showCode && (
          <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
            <div className={"rounded-xl shadow-2xl p-6 max-w-4xl w-full relative " + themes[theme].panel}>
              ...
              <pre className="bg-gray-100 rounded-lg p-4 text-xs overflow-auto max-h-[60vh] border border-gray-200">
